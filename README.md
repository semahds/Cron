<img width="150" height="300" alt="cron_w_text" src="https://github.com/user-attachments/assets/8e7e9de5-8899-432a-aaeb-9066ddf99bb6" />

> **Cron** - это мобильное приложение, которое позволяет школьникам вести расписание и отслеживать оценки. Расписание можно добавить вручную или отсканировать с фотографии через OCR. Приложение заранее
> уведомляет о предстоящих уроках, синхронизирует данные с облаком и предоставляет доступ к расписанию через Telegram-бот - в том числе к расписанию друзей, которых можно добавить в приложении.
>
> Название отсылает к Unix-утилите cron - стандартному планировщику задач, который запускает процессы по расписанию. Это отражает главную идею приложения: всё строится вокруг времени и расписания. Также
> бэкенд Cron работает на Cloudflare Workers, где для фоновых задач используются Cron Triggers.
>
> Логотип - звезда с округлыми концами. Шесть лучей напоминают стрелки часов, что отсылает к теме времени и расписания. В то же время форма логотипа напоминает знак
> звёздочки, который в синтаксисе cron означает «каждый»: каждую минуту, каждый час, каждый день.
